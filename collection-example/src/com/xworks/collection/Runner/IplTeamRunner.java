...
public class IplTeamRunner {
    public static void main(String[] args) {
        ...
        IplTeamDto ipl=new IplTeamDto("Rcb",23,"United Sperits","Qatar airways",83);
        System.out.println("total before"+collection.size());
        Iterator<IplTeamDto> iterator= collection.iterator();
        while (iterator.hasNext()){
            IplTeamDto value=iterator.next();
            if (value.getSponser().equals("jio")){
                System.out.println("team :"+value.getTeamName());
                iterator.remove();
            }
        }
        System.out.println("total after "+collection.size());
